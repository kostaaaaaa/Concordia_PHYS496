...
class Lattice:
    ...
    def plot_lattice_WC_BZ_comparison(self, n, save=False):
        ...
        if self.lattice_type=="Hexagon":
            ...
            ax3.annotate(f'({pointx},{pointy})', xy=(pointx,pointy), xytext=(pointx+2,pointy+2))
    ...
    def animate_lattice_WC_BZ_comparison(self, n, save=False):
        """Animating the difference in Wigner Cell and Brillouin Zone for a range of n from 1 to user input."""
        ...
        def update(frame):
            ...
            if self.lattice_type == "Hexagon":
                points, points2 = np.multiply(self.generate_reciprocal_points(), (1 / current_n))
                ax2.scatter(points[:, 0], points[:, 1], color=(0.1, 0.2, 0.5, 0.5), s=50)
                bz_edges = self.generate_BZEdges(self.reciprocal_vectors[0], self.reciprocal_vectors[1])
                for i in range(len(bz_edges)):
                    start = (1 / current_n) * bz_edges[i]
                    end = (1 / current_n) * bz_edges[(i + 1) % len(bz_edges)]
                    ax2.plot([start[0], end[0]], [start[1], end[1]], '-', color=(0.1, 0.2, 0.5, 0.8))
                ...
                WC_edges = [
                [current_n * 0.5, current_n * np.sqrt(3) / 6],
                [current_n * 0.5, current_n * -np.sqrt(3) / 6],
                [0, current_n * -np.sqrt(3) / 3],
                [current_n * -0.5, current_n * -np.sqrt(3) / 6],
                [current_n * -0.5, current_n * np.sqrt(3) / 6],
                [0, current_n * np.sqrt(3) / 3]
                ]
                ...
                for i in range(len(WC_edges)):
                    ...
                    ax1.plot([start[0], end[0]], [start[1], end[1]], '-', color=(0.1, 0.2, 0.5, 0.8))
            
                s_vals = np.arange(0, 10, 1)
                ax3.scatter(2 * s_vals ** 2, s_vals)
                ax3.scatter(2 * current_n ** 2, current_n, color=(0.5, 0.2, 0.1, 1))
            
                pointy = current_n
                pointx = 2 * current_n ** 2
                ax3.annotate(f'({pointx},{pointy})', xy=(pointx, pointy), xytext=(pointx+0.2,pointy+0.2))

            ax1.set_title(f'{self.lattice_type} Lattice with Wigner Cell (s={current_n})')
            ax1.set_xlabel("x")
            ax1.set_ylabel("y")
            ax2.set_title(f'{self.lattice_type} Reciprocal Lattice with Brillouin Zone (s={current_n})')
            ax2.set_xlabel(r'$k_x$')
            ax2.set_ylabel(r'$k_y$')
            ax3.set_title("Atoms per unit cell")
            ax3.set_xlabel("Atoms")
        ...
        anim = FuncAnimation(fig, update, frames=n, init_func=init, blit=False, repeat=False, interval=2000)

        if save:
            anim.save('lattice_animation.mp4', fps=5, extra_args=['-vcodec', 'libx264'])
